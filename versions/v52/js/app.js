...
function toggleFaq(el) {
  ...
  el.querySelector('span:last-child').textContent = answer.classList.contains('show') ? '−' : '+';
}
...
function toggleDetails() {
  ...
  if (typeof t === 'function') {
    btn.innerHTML = isOpen
      ? t('footer.showLess') + ' <span class="arrow">↑</span>'
      : t('footer.showMore') + ' <span class="arrow">↓</span>';
  } else {
...
// ── Quick Key Login (from hero) ──
function quickKeyLogin(e) {
  e.preventDefault();
  const key = document.getElementById('heroKeyInput').value.trim();
  if (!key || !key.startsWith('seo_')) return false;
  saveKey(key);
  switchToDashboard(key);
  return false;
}
...
async function switchToDashboard(key, balance) {
  ...
  // Hide free-tier marketing sections
  var hero=document.getElementById('dashHero');if(hero)hero.style.display='none';
  var sample=document.getElementById('dashSampleReport');if(sample)sample.style.display='none';
  // Show dashboard panel + keep page content visible
...
function dashLogout() {
  ...
  // Restore free-tier sections
  var hero=document.getElementById('dashHero');if(hero)hero.style.display='';
  var sample=document.getElementById('dashSampleReport');if(sample)sample.style.display='';
...
function renderResults(report, url) {
  ...
  container.innerHTML = `
    <div class="report-card">
      ...
      <!-- Email Collection (free users) -->
      ${!isPaid ? `
      <div class="email-collect-section" style="margin-top:16px;padding:16px;background:rgba(137,180,250,.06);border:1px solid rgba(137,180,250,.15);border-radius:10px;text-align:center">
        <div style="font-size:14px;margin-bottom:8px">📬 Want the full report + fix templates sent to your inbox?</div>
        <div style="display:flex;gap:8px;max-width:400px;margin:0 auto">
          <input type="email" id="emailCollectInput" placeholder="your@email.com" style="flex:1;padding:10px 14px;border-radius:8px;border:1px solid rgba(137,180,250,.2);background:rgba(24,24,37,.8);color:#cdd6f4;font-size:13px;outline:none;font-family:inherit">
          <button onclick="submitEmailReport()" style="padding:10px 20px;border-radius:8px;border:none;background:linear-gradient(135deg,#89b4fa,#b4befe);color:#11111b;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap">Send Me the Report</button>
        </div>
        <div id="emailCollectStatus" style="font-size:11px;color:#585b70;margin-top:8px"></div>
      </div>
      ` : ''}
...
// ── Email Collection (free report to inbox) ──
async function submitEmailReport() {
  const email = document.getElementById('emailCollectInput').value.trim();
  const status = document.getElementById('emailCollectStatus');
  ...
  status.style.color = '#585b70';
  const report = window._lastReport;
  try {
    const API_BASE = getApiBase();
    const res = await fetch(API_BASE + '/v1/seo/subscribe-email', {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        email: email,
        report: report || {},
    ...
    const data = await res.json();
    if (data.success) {
      status.textContent = '✅ Check your inbox — the full report is on its way!';
      status.style.color = '#a6e3a1';
      document.getElementById('emailCollectInput').value = '';
    } else {
...
const PLAN_INFO = {
  starter: { price: 14.9, name: 'Starter Pack', scans: 5, pkg: 'seo_starter',
    benefits: ['Full 24-point SEO audit with every scan',"Copy-paste fix templates for each issue",'Downloadable PDF report with AI Visibility check','5 scans — use on any URL, anytime'] },
  growth: { price: 49, name: 'Growth Pack', scans: 25, pkg: 'seo_growth',
    benefits: ['Everything in Starter Pack','Weekly automated SEO & AI Visibility reports','Complete 24-point audit with priority ranking','Historical score trends and PDF export','Register URLs for weekly auto-monitoring','Actionable fix templates with code snippets'] },
  pro: { price: 149, name: 'Pro Pack', scans: 150, pkg: 'seo_pro',
    benefits: ['Everything in Growth Pack','150 scans — perfect for 1+ year of weekly monitoring','Score trend dashboard with charts','AI Visibility tracking (coming soon)','Register unlimited URLs for weekly push','All future features included at no extra cost'] },
  monthly: { price: 9.9, name: 'Weekly Subscription', scans: '∞', pkg: 'seo_monthly',
    benefits: ['One URL auto-scanned every week','Email report delivered every Monday','Score trend tracking over time','30 days per payment cycle','Cancel anytime — no lock-in, no penalty'] }
};
...
function openSubscribe(tier) {
  ...
  const report = window._lastReport;
  const rurl = report && report.url || '';
  const domain = rurl ? (rurl.indexOf('://') > -1 ? new URL(rurl).hostname : rurl) : '';
  const isMonthly = tier === 'monthly';
  const benefits = getModalBenefits(tier);
  const planName = t('pricing.' + (isMonthly ? 'weekly' : tier) + '.name');
  document.getElementById('subscribeContent').innerHTML = `
    <h3>${isMonthly ? '📬' : tier === 'starter' ? '📊' : tier === 'growth' ? '🔧' : '🏆'} ${planName}</h3>
    <div class="price-tag">$${p.price} <small style="font-size:12px;color:#585b70">${isMonthly ? t('modal.per_month') : '— ' + p.scans + t('modal.scans')}</small></div>
    <ul class="benefits-list">
      ${benefits.map(b => '<li>'+b+'</li>').join('')}
    </ul>
    <label>${t('modal.email_label')}</label>
    <input type="email" id="subEmail" placeholder="${t('modal.email_placeholder')}" required>
    <div id="subStatus" class="status-msg"></div>
    <button class="btn-submit" id="payBtn" onclick="payWithUsdt('${tier}')" style="background:linear-gradient(135deg,#a6e3a1,#94e2d5);color:#11111b">${t('modal.pay_btn')} $${p.price}${isMonthly ? t('modal.per_month') : ''}</button>
    <div class="detail">${isMonthly
      ? t('modal.detail.monthly')
      : tier === 'growth' ? t('modal.detail.growth')
      : t('modal.detail.pack')}</div>
  `;
    ...
    if (data.success) {
      ...
      content.innerHTML = `
        <div style="text-align:center">
          ...
          <h3 style="font-size:15px;color:#cdd6f4;margin-bottom:4px">${t('modal.order_created')}</h3>
        ...
        <div style="background:rgba(24,24,37,.6);border:1px solid rgba(137,180,250,.15);border-radius:8px;padding:14px;text-align:center;margin-top:10px">
          <div style="font-size:10px;color:#585b70;margin-bottom:4px">${t('modal.send_to')}</div>
          ...
          <button onclick="copyWallet()" id="usdtCopyBtn" style="padding:6px 16px;border-radius:6px;border:none;background:rgba(137,180,250,.12);color:#89b4fa;font-size:11px;cursor:pointer;font-family:inherit;margin-bottom:6px">${t('modal.copy_addr')}</button>
          <div style="font-size:10px;color:#585b70">${t('modal.waiting')} <span id="usdtTimer" style="color:#f9e2af">0s</span></div>
          <div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(137,180,250,.08);text-align:left;font-size:10px;color:#45475a;line-height:1.6">
            📌 <strong style="color:#585b70">${t('modal.steps_title')}</strong><br>
            ${t('modal.step1')}<br>
            ${t('modal.step2')}<br>
            ${t('modal.step3').replace('${amount}', d.amount)}<br>
            ${t('modal.step4').replace('${email}', email)}
          </div>
          <div style="margin-top:6px;font-size:10px;color:#585b70">
            🔗 <a href="https://tronscan.org/#/address/${d.wallet}" target="_blank" rel="noopener" style="color:#89b4fa;text-decoration:none">${t('modal.view_tronscan')}</a>
          </div>
        </div>
        <div class="detail" style="margin-top:6px;text-align:center">Order: ${d.order_id} · <a href="javascript:void(0)" onclick="closeSubscribe()" style="color:#585b70">${t('modal.close')}</a></div>
      `;
            ...
            if (!isPending) {
              ...
              content.innerHTML = `
                <div style="text-align:center">
                  ...
                  <div style="font-size:14px;color:#a6e3a1;font-weight:600">${t('modal.paid_title')}</div>
                  <div style="font-size:11px;color:#585b70;margin-top:4px">${t('modal.paid_desc').replace('${email}', email)}</div>
                  <button onclick="closeSubscribe()" class="btn-submit" style="margin-top:12px;background:linear-gradient(135deg,#a6e3a1,#94e2d5);color:#11111b;max-width:200px">${t('modal.paid_done')}</button>
                </div>
              `;
            }
          }
...
function renderFullPaidResults(report, key) {
  ...
  if (reportUrl && !reportUrl.startsWith('http://') && !reportUrl.startsWith('https://')) reportUrl = 'https://' + reportUrl;
  const domain = reportUrl ? new URL(reportUrl).hostname : '';
  container.innerHTML =
    '<div class="report-card">' +
      '<div class="score-wrap">' +
        '<div class="score-ring" style="background:conic-gradient(' + scoreColor + ' 0deg ' + degrees + 'deg, #313244 ' + degrees + 'deg 360deg)">' +
...
function renderMultiResults(results, key) {
  ...
  const domain = r.url ? (r.url.indexOf('://') > -1 ? new URL(r.url).hostname : r.url) : '';
